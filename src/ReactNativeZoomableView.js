...
class ReactNativeZoomableView extends Component {
    ...
    _getBoundOffsetValue(axis: 'x'|'y', offsetValue: number, containerSize: number, elementSize: number, zoomLevel: number) {
        ...
        const startBorder = (elementSize - (axis === 'x' ? windowWidth : containerSize)) / 2;
        const endBorder = -startBorder;

        const distanceToStart = zoomLevelOffsetValue - startBorder + (axis === 'y' ? this.props.hardOffsetTop : 0);
        const distanceToEnd = endBorder - zoomLevelOffsetValue

        // set context for callback events
        this._setContextStateDistances(axis, distanceToStart, distanceToEnd);

        if (axis === 'x') {
            if (elementSize < windowWidth) {
                return ((containerSize / 2) - (elementSize / 2) / zoomLevel);
            }

            if (distanceToStart > 0) {
                return startBorder / zoomLevel;
            }
        }

        if (axis === 'y') {
            if (elementSize > containerSize) {
                const isTresholdBreaked = distanceToEnd < 0;
                if (this.isAfterBottomEdge !== isTresholdBreaked) {
                    this.props.onTresholdBreak(isTresholdBreaked);
                    this.isAfterBottomEdge = isTresholdBreaked;
                }
            }
            if (elementSize < containerSize + this.props.hardOffsetTop) {
                return endBorder / zoomLevel
            }

            if (distanceToStart > 0) {
                return (startBorder - this.props.hardOffsetTop) / zoomLevel;
            }
        }
